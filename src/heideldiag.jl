...
"""
function heideldiag(x::AbstractVector{<:Real}; alpha::Real=0.05, eps::Real=0.1,
                             start::Int=1, kwargs...)
  n = length(x)
  ...
  return (burnin=i + start - 2, stationarity=converged, pvalue=pvalue, mean=ybar, halfwidth=halfwidth, test=passed)
end
...
function pcramer(q::Real)
    ...
    for k in 0:3
        c1 = 4.0 * k + 1.0
        c2 = c1^2 / (16.0 * q)
        p += SpecialFunctions.gamma(k + 0.5) / factorial(k) * sqrt(c1) * exp(-c2) * SpecialFunctions.besselk(0.25, c2)
    end
